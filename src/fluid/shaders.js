...
fn main(@builtin(global_invocation_id) global_id : vec3<u32>) {
    ...
    // 🔥 MULTICOLOR LOGIC
    let speed = length(uMouse.vel) * 10.0;
    let hue = fract(uTime * 0.05 + speed * 0.15);

    let col_start = palette(
      hue,
      vec3(0.5),
      vec3(0.5),
      vec3(1.0),
...
fn main(@builtin(global_invocation_id) global_id : vec3<u32>) {
    ...
    // 🔥 COLOR EVOLUTION
    let rotated = vec3(
      bilerp.x * 0.99 + bilerp.y * 0.01,
      bilerp.y * 0.99 + bilerp.z * 0.01,
      bilerp.z * 0.99 + bilerp.x * 0.01
    );

    x_out[index] = rotated.x;
    y_out[index] = rotated.y;
    z_out[index] = rotated.z;
}`;
...
export const vorticityConfinmentShader = /* wgsl */ `      
...
export const checkerboardShader = /* wgsl */ `    
