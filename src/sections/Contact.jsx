...
const Contact = () => {
  ...
  const [logs, setLogs] = useState(["AWAITING SIGNAL..."]);
  ...
  const sendEmail = async (e) => {
    ...
    setLogs(prev => [...prev, "CALIBRATING DIMENSION..."]);
    ...
    try {
      ...
      setLogs(prev => [...prev, "SIGNAL RECEIVED IN EARTH-28."]);
      setStatus("success");
      setEffectText("BOOM!");
      gsap.fromTo('.comic-effect', 
      ...
      setLogs(prev => [...prev, "PORTAL COLLAPSED. TRY AGAIN."]);
      setStatus("error");
      setTimeout(() => setStatus("idle"), 5000);
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="contact-panel lg:col-span-7 h-full">
            <ComicPanel theme="dark" rotation="-1deg" className="h-full p-6 md:p-10 relative overflow-hidden bg-[var(--color-deep-navy)]" style={{ borderColor: 'var(--color-portal-cyan)' }}>
              ...
              <h3 className="font-display text-3xl mb-8 text-[var(--color-dimension-magenta)]">INITIATE TRANSMISSION</h3>
              
              <form ref={formRef} onSubmit={sendEmail} className="space-y-6 relative z-10">
                <div className="space-y-2">
                  <label className="font-label font-bold uppercase text-[var(--color-text-muted-dark)] text-xs">Operator Alias</label>
                  <input
                    type="text"
                    name="from_name"
                    required
                ...
                <div className="space-y-2">
                  <label className="font-label font-bold uppercase text-[var(--color-text-muted-dark)] text-xs">Return Coordinates</label>
                  <input
                    type="email"
                    name="from_email"
                    required
                ...
                <div className="space-y-2">
                  <label className="font-label font-bold uppercase text-[var(--color-text-muted-dark)] text-xs">Payload</label>
                  <textarea
                    name="message"
                    rows="4"
                    required
                ...
                <div className="pt-4 relative">
                  ...
                  >
                    {status === "sending" ? "TRANSMITTING..." : status === "success" ? "TRANSMISSION SENT!" : "FIRE SIGNAL"}
                  </InkButton>
          ...
          <div className="lg:col-span-5 flex flex-col gap-8 h-full">
            ...
            <div className="contact-panel flex-grow">
              <ComicPanel theme="light" rotation="1deg" className="h-full p-6 md:p-8 bg-[var(--color-paper-light)]">
                <div className="border-b-4 border-[var(--color-ink-black)] pb-4 mb-6">
                  <h3 className="font-display text-4xl text-[var(--color-ink-black)] leading-none">TARGET IDENTIFIED</h3>
                </div>
                
                <div className="space-y-6">
                  <div>
                    <span className="font-label uppercase font-bold text-xs text-[var(--color-pencil-gray)] block mb-1">Codename</span>
                    <span className="font-display text-2xl text-[var(--color-ink-black)]">{profileData.alias}</span>
                  </div>
                  <div>
                    <span className="font-label uppercase font-bold text-xs text-[var(--color-pencil-gray)] block mb-1">Direct Link</span>
                  ...
                  <div>
                    <span className="font-label uppercase font-bold text-xs text-[var(--color-pencil-gray)] block mb-1">Known Networks</span>
                  ...
                  <div>
                    <span className="font-label uppercase font-bold text-xs text-[var(--color-pencil-gray)] block mb-1">Coordinates</span>
                    <span className="font-mono font-bold text-sm text-[var(--color-ink-black)]">EARTH-28 [IN]</span>
                  </div>
                </div>
              </ComicPanel>
            </div>
            ...
            <div className="contact-panel h-48">
              <ComicPanel theme="dark" rotation="-2deg" className="h-full p-4 bg-black border-4 border-[var(--color-signal-red)]">
                <div className="font-mono text-xs text-[var(--color-signal-red)] mb-2 border-b border-[var(--color-signal-red)]/30 pb-1">
                  PORTAL_LOG_v1.0
                </div>
                <div className="space-y-1 font-mono text-sm h-full overflow-hidden flex flex-col justify-end pb-4">
                  {logs.map((log, i) => (
                    <div key={i} className={i === logs.length - 1 ? "text-[var(--color-comic-yellow)]" : "text-[var(--color-signal-red)] opacity-60"}>
                      {">"} {log}
                    </div>
                  ))}
                  {(status === "sending" || status === "idle") && (
                    <div className="text-[var(--color-comic-yellow)] animate-pulse">_</div>
                  )}
                </div>
              </ComicPanel>
            </div>
