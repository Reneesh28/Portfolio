    ...
    gsap.fromTo('.approved-stamp',
      { scale: 3, opacity: 0, rotation: 15 },
      {
        ...
        onComplete: () => {
          ...
          gsap.to('.folder-container', {
            x: 'random(-6,6)', y: 'random(-5,5)', duration: 0.04, repeat: 4, yoyo: true,
            onComplete: () => gsap.set('.folder-container', { x: 0, y: 0 }),
          });
        },
      }
    );
    ...
    <ComicSpread id="education" className="bg-[var(--color-deep-navy)] z-10" ref={sectionRef}>
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto flex flex-col items-center">
        ...
        <div className="folder-container w-full relative pt-12">
          ...
          <div className="absolute top-0 left-4 md:left-8 flex gap-2 z-0">
            {educationData.map((edu, idx) => (
              <button
                ...
                className={`font-label uppercase font-bold text-sm md:text-base px-6 py-3 rounded-t-lg transition-all duration-300 transform origin-bottom border-t-2 border-l-2 border-r-2 ${activeTab === idx
          ...
          {/* Folder Body */}
          <TiltCard maxTilt={3} scaleOnHover={1.005}>
